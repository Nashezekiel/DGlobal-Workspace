...
import { Archive, Calendar, Loader2, Search, X, Link as LinkIcon, ImageIcon } from 'lucide-react'
...
export default function AdminArchivedPage() {
    ...
    const fetchArchivedTasks = async () => {
      ...
      if (taskIds.length > 0) {
        const { data: subsData } = await supabase
          .from('submissions')
          .select('task_id, content, submitted_at, worker_id, status')
          .in('task_id', taskIds)
          .eq('status', 'approved')
      ...
      const enriched: ArchivedTask[] = (tasksData || []).map((t: ArchivedTask) => ({
        ...t,
  ...
  const filtered = useMemo(() => {
    return tasks.filter((task) => {
      ...
      if (dateFrom) {
        ...
        if (new Date(taskDate) < new Date(dateFrom)) return false
      }
      if (dateTo) {
        const taskDate = task.updated_at || task.created_at
        if (new Date(taskDate) > new Date(dateTo + 'T23:59:59')) return false
      }
      return true
    })
  }, [tasks, search, priorityFilter, dateFrom, dateTo])
  ...
  const hasActiveFilters = search || priorityFilter !== 'all' || dateFrom || dateTo

  const getAssigneeName = (task: ArchivedTask) => {
    const profiles = task.profiles as { full_name: string } | { full_name: string }[] | null | undefined
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="All completed and archived tasks across the team."
      />
      ...
      <div className="bg-white rounded-xl border shadow-sm p-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          {/* Clear */}
          {hasActiveFilters && (
            <Button variant="ghost" size="sm" onClick={clearFilters} className="gap-1.5 text-gray-500 hover:text-gray-900">
              <X className="h-4 w-4" />
              Clear
            </Button>
          )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-xl border shadow-sm">
          ...
          <p className="text-gray-500 max-w-sm">
            Tasks that are approved and completed will appear here.
          </p>
        </div>
      ) : filtered.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      <TableCell className="font-medium text-gray-900">
                        ...
                        <div className="text-xs text-gray-400 mt-0.5">ID: {task.id.slice(0, 8)}</div>
                      </TableCell>
              ...
              <>
                ...
                <div className="space-y-4 mt-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Badge className="text-xs px-2 py-0.5 font-medium bg-purple-100 text-purple-700">
                      Archived
                    </Badge>
                  </div>
                  ...
                  {selectedTask.due_date && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600">
                        Due: {new Date(selectedTask.due_date).toLocaleDateString(undefined, { dateStyle: 'long' })}
                      </span>
                    </div>
                  )}
