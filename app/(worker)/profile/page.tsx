              ...
              {user?.id && (
                <div className="flex items-start gap-3 text-sm pt-2 border-t border-gray-100">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-500 mb-0.5 font-medium">Member ID</p>
                    <div className="flex items-center gap-2">
                      <code className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded font-mono tracking-wide">
                        DGW-{user.id.slice(0, 8).toUpperCase()}
                      </code>
                      <button
                        type="button"
                        title="Copy Member ID"
                        onClick={() => {
                          navigator.clipboard.writeText(`DGW-${user!.id.slice(0, 8).toUpperCase()}`)
                          setCopiedId(true)
